...
Note that the value of the field as put as-is.

Before version 0.3.0 it was possible to use the B<$'FIELD> notation to produce
a shell-compatible escaped string of the field value. This has been removed and
isn't supported anymore, as field values are now always replaced as-in B<after>
the trigger was parsed for execution/splitted into an argv, avoiding any
possible splitting issue.


Also note that the trigger does not directly support shell command lines. If
...
However, be aware that your shell will be parsing the command-line, and if the
value of a field contained special characters this could cause issues. (Using a
script might be a way around it.)
